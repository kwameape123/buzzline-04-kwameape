...
The concept of producers don't change from buzzline-03-case - they write the same information to a Kafka topics or can write data to a CSV or JSON file. In this project we build on the concept of consumers in the previous modules. We modify consumers to add real time visualization capabilities.
...
The producer writes information about a message and its author to a kafka topic. The consumer then consumes or reads this data to perform analysis and visualization the result of this analysis. The consumer in this project is going to do two things we regards to visualization;
1. Visualize the frequency of keywords.(Bar Chart.)
2. Visualize the percentage frequency of keywords.(Bar Chart.)
3. Monitor the sentiment ratings of an author overtime.This is possible since every message generated by an author is timestamped.(Line Chart.)

## First, Verify tool installation and Setup.

Before starting, ensure you have completed the setup tasks for;
1. VS Code.
...
1. Once the tools are installed, project is copied/forked into my GitHub account
   and my version of this project is cloned to my local machine to run and experiment with.
2. It is name `buzzline-04-arnold`.
...
---
## Task 3. Examine Project Producer

Examine the code of the project producer to understand and identify;

1. The data that is to be sent to the kafka topic.
2. Information about our kafka topic. 

Producer name is **project_producer_case**

## Task 4. Develop Project Consumer

Base on the information you gathered from the producers and the goals of the project,
develop a consumer. The consumer will be name **project_consumer_arnold**

---

## Task 5. Start a (Kafka-based) JSON Streaming Application
